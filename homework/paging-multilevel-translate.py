...
class OS:
    ...
    def initPageDir(self, whichPage):
        # self.dumpPage(whichPage)
        whichByte = whichPage << self.pageBits
        # print '  init page %d (%08x)' % (whichPage, whichPage)
        for i in range(whichByte, whichByte + self.pageSize):
            # print '    setting byte', i, 'to', 0x7f
            self.memory[i] = 0x7f
        # self.dumpPage(whichPage)
    ...
    def translate(self, pid, virtualAddr):
        # print '**TRANSLATE**', pid, virtualAddr
        ...
        if valid == 1:
            ...
            # print '    --GET PTE--', valid, pfn
            if valid == 1:
                offset = (virtualAddr & self.OFFSET_MASK)
                paddr  = (pfn << self.pageBits) | offset
                # print '      --RETURN--', paddr, 'from PFN', pfn, self.pageBits, offset
                return paddr
            else:
                # print '    --INVALID PTE--'
                return -2
...
for i in range(0, options.num):
    ...
    if options.solve == True:
        ...
        print 'Virtual address %04x: translates to what Physical Address (and hence fetches what Value), or a Fault?' % vaddr
