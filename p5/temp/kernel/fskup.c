...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
      bfree(ip->dev, (ip->addrs[i] &0x00FFFFFF));//CHANGED
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, (ip->addrs[NDIRECT] &0x00FFFFFF));//CHANGED
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
      if(a[j])
        bfree(ip->dev, (a[j]&0x00FFFFFF));//CHANGED
    }
    brelse(bp);
    bfree(ip->dev, (ip->addrs[NDIRECT] &0x00FFFFFF));//CHANGED
    ip->addrs[NDIRECT] = 0;
  }
...
void
stati(struct inode *ip, struct stat *st) //TODO CHANGE ME
{
  st->dev = ip->dev;
  st->ino = ip->inum;
  st->type = ip->type;
  ...
  if(ip->type == T_CHECKED) {
	...
	for(j = 0; j < BSIZE; j++) {
	  checksum2 ^= bp->data[j];
	  //checksum2 = (a[j] & 0xFF000000);
	  //checksum2 = (checksum >> 24);
	  //checksum ^= checksum2;
	}
	checksum ^= checksum2;
	brelse(bp);
	...
	//checksum ^= checksum2;
	//checksum2 = ip->addrs[i] & 0xFF000000;
...
int
readi(struct inode *ip, char *dst, uint off, uint n) //TODO CHANGE ME
{
  uint tot, m, addr;
  struct buf *bp, *bp2;
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    //ADDED
    bp = bread(ip->dev, addr); //altered bmap(ip, off/BSIZE) changed to addr
	...
	a = (uint*)bp2->data; //TODO GET ADDRESS BLOCK INSTEAD
	checksum = a[bn] & 0xFF000000;
        checksum = (checksum >> 24);
	brelse(bp2);
      } else {
	cprintf("Direct\n");
        checksum = ip->addrs[off/BSIZE] & 0xFF000000;
        checksum = (checksum >> 24);
      }
      cprintf("checksum int = %d \n", checksum);
...
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n) //TODO CHANGE ME
{
  uint tot, m, addr;
  struct buf *bp, *bp2;
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
	...
	cprintf("Indirect\n");
        bp2 = bread(ip->dev, ip->addrs[NDIRECT]);
	a = (uint*)bp2->data; //TODO GET ADDRESS BLOCK INSTEAD
	a[bn] &= 0x00FFFFFF;   // <----CHANGED MAY NOT NEED THIS    
	a[bn] |= checksum2;//(checksum << 24);
        bwrite(bp2);
        brelse(bp2);
      } else {
	cprintf("Direct\n");
        ip->addrs[off/BSIZE] &= 0x00FFFFFF; //ADDED
        ip->addrs[off/BSIZE] |= checksum2; //(checksum << 24);
