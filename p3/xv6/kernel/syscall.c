...
{
  if((addr >= p->sz && addr < p->stack_top) || (addr+4 > p->sz && addr+4 < p->stack_top)){
//	cprintf("FETCHERROR\n");
    return -1;
...
//    cprintf("FETCHSTR ERROR\n");
    //return -1;
  }
  //cprintf("%d address fetchstr\n",addr);
  *pp = (char*)addr;
  ep = (char*)p->sz;
  ...
  for(s = *pp; s < ep; s++)
   ...
   else if((addr>=p->stack_top)&&(addr<USERTOP-1)){
  for(s = *pp; s>=(char*)p->stack_top;s--)
    if(*s ==0)
	return  *pp-s;
  }
  return -1;
